# Skill Installer - 快速测试指南

> 测试JarvisMolt技能自动学习流程

---

## 🎯 测试场景

模拟用户在OpenClaw中学习lark技能的完整流程。

---

## 🚀 快速测试

### 准备工作

```bash
cd Skills/skill-installer

# 确保脚本可执行
chmod +x agent.js
```

---

### 场景1：首次学习技能（需要授权码）

```bash
# 步骤1：用户输入学习指令
node agent.js "从 https://gitee.com/bobsharon/JarvisMolt-Skills 学习lark技能"

# 预期输出：
# 提示输入授权码
```

**预期交互**：
```
╔═══════════════════════════════════════════════════╗
║     Skill Installer - JarvisMolt技能安装器        ║
╚═══════════════════════════════════════════════════╝

🎯 目标技能: lark
🔗 Gitee仓库: https://gitee.com/bobsharon/JarvisMolt-Skills

📋 步骤1: 请输入授权码...

🔐 该技能需要授权码才能使用
   请输入授权码（从技能提供者处获取）:

═══════════════════════════════════════════════════
Agent返回结果:
═══════════════════════════════════════════════════

该技能需要授权码才能使用。

请输入授权码：（格式：XXXX-XXXX-XXXX-XXXX-XX）

获取授权码请联系技能提供者。

⚠️  需要用户输入
Context: {
  action: 'verify-license',
  skillName: 'lark',
  giteeUrl: 'https://gitee.com/bobsharon/JarvisMolt-Skills'
}
```

---

### 场景2：生成测试授权码

```bash
cd ../../scripts

# 生成测试授权码
node generate-code.js lark --type trial --days 7

# 复制生成的授权码（例如：ABCD3456-789N-PQRS-TUVW-XY）
```

---

### 场景3：模拟用户已有授权

> **注意**：即使本地存在授权缓存，agent 也会强制要求重新输入授权码（不使用缓存）。
> 缓存仅用于 `查看技能授权` 命令展示已激活的技能信息。

**方式A：手动创建授权缓存**

```bash
# 创建授权缓存目录
mkdir -p ~/.openclaw/licenses

# 创建授权文件
cat > ~/.openclaw/licenses/lark.json << 'EOF'
{
  "skill": "lark",
  "code": "TEST-CODE-FOR-DEMO",
  "activatedAt": 1738743600000,
  "expiresAt": 1770279600000,
  "type": "permanent",
  "tier": "standard"
}
EOF

# 再次运行agent — 仍然会提示输入授权码
cd Skills/skill-installer
node agent.js "从 https://gitee.com/bobsharon/JarvisMolt-Skills 学习lark技能"
```

**预期输出**：
```
╔═══════════════════════════════════════════════════╗
║     Skill Installer - JarvisMolt技能安装器        ║
╚═══════════════════════════════════════════════════╝

🎯 目标技能: lark
🔗 Gitee仓库: https://gitee.com/bobsharon/JarvisMolt-Skills

📋 步骤1: 请输入授权码...

🔐 该技能需要授权码才能使用
   请输入授权码（从技能提供者处获取）:

═══════════════════════════════════════════════════
Agent返回结果:
═══════════════════════════════════════════════════

该技能需要授权码才能使用。

请输入授权码：（格式：XXXX-XXXX-XXXX-XXXX-XX）

获取授权码请联系技能提供者。

⚠️  需要用户输入
Context: {
  action: 'verify-license',
  skillName: 'lark',
  giteeUrl: 'https://gitee.com/bobsharon/JarvisMolt-Skills'
}
```

---

### 场景4：查看已授权技能

```bash
node agent.js "查看我的技能授权"
```

**预期输出**：
```
已授权技能列表：

┌─────────────┬──────────────────────┬──────────┬──────────────┐
│ 技能名称     │ 授权类型              │ 剩余天数  │ 状态         │
├─────────────┼──────────────────────┼──────────┼──────────────┤
│ lark │ permanent            │ 永久      │ ✓ 有效       │
└─────────────┴──────────────────────┴──────────┴──────────────┘
```

---

### 场景5：移除技能

```bash
node agent.js "移除lark技能"
```

**预期输出**：
```
✓ 技能"lark"已移除。

授权信息已保留，可以随时重新安装。
```

---

## 🧪 完整测试流程

### 测试脚本：`test-flow.sh`

```bash
#!/bin/bash

echo "========================================="
echo "Skill Installer 完整测试流程"
echo "========================================="

# 1. 清理环境
echo -e "\n1. 清理测试环境..."
rm -rf ~/.openclaw/licenses/lark.json
rm -rf ~/.openclaw/skills/lark

# 2. 生成测试授权码
echo -e "\n2. 生成测试授权码..."
cd ../../scripts
CODE=$(node generate-code.js lark --type trial --days 7 2>&1 | grep "验证码:" | awk '{print $2}')
echo "   授权码: $CODE"

# 3. 创建授权缓存（模拟用户已输入授权码）
echo -e "\n3. 创建授权缓存..."
mkdir -p ~/.openclaw/licenses
cat > ~/.openclaw/licenses/lark.json << EOF
{
  "skill": "lark",
  "code": "$CODE",
  "activatedAt": $(date +%s)000,
  "expiresAt": $(date -v+7d +%s)000,
  "type": "trial"
}
EOF

# 4. 测试学习技能
echo -e "\n4. 测试学习技能..."
cd Skills/skill-installer
node agent.js "从 https://gitee.com/bobsharon/JarvisMolt-Skills 学习lark技能"

# 5. 验证安装
echo -e "\n5. 验证技能安装..."
if [ -d ~/.openclaw/skills/lark ]; then
    echo "   ✓ 技能目录存在"
    echo "   ✓ 文件列表:"
    ls -la ~/.openclaw/skills/lark/
else
    echo "   ✗ 技能未安装"
fi

# 6. 查看授权列表
echo -e "\n6. 查看授权列表..."
node agent.js "查看我的技能授权"

echo -e "\n========================================="
echo "测试完成！"
echo "========================================="
```

### 运行测试

```bash
# 保存上述脚本
cat > Skills/skill-installer/test-flow.sh << 'EOF'
[粘贴上述脚本内容]
EOF

# 赋予执行权限
chmod +x test-flow.sh

# 运行测试
./test-flow.sh
```

---

## 📊 测试检查清单

### 功能测试

- [ ] 解析用户输入（学习技能）
- [ ] 提示输入授权码
- [ ] 验证授权码
- [ ] 从安全服务器下载技能包
- [ ] 安装技能到~/.openclaw/skills/
- [ ] 清理临时文件
- [ ] 查看授权列表
- [ ] 移除技能

### 边界情况测试

- [ ] 授权码不存在
- [ ] 授权码已过期
- [ ] 网络连接失败
- [ ] 技能目录不存在
- [ ] 权限不足

---

## 🐛 常见问题

### Q1: 下载失败

**问题**：技能包下载超时或连接失败

**解决**：
```bash
# 检查网络连接
curl -I https://your-api-server.com

# 确保 API 服务器可达
# 如果在国内，检查是否需要代理
```

### Q2: 找不到验证码脚本

**问题**：`验证码验证系统未找到`

**解决**：
```bash
# 确保scripts目录存在
ls -la scripts/validate-code.js

# 如果不存在，回到项目根目录查看
```

### Q3: 权限错误

**问题**：`EACCES: permission denied`

**解决**：
```bash
# 确保目录权限正确
chmod 755 ~/.openclaw
chmod 755 ~/.openclaw/skills
chmod 700 ~/.openclaw/licenses
```

---

## 🔄 集成到OpenClaw

### 步骤1：安装skill-installer

```bash
# 复制到OpenClaw技能目录
cp -r Skills/skill-installer \
      ~/.openclaw/skills/

# 重启OpenClaw
openclaw gateway restart
```

### 步骤2：在OpenClaw中使用

```
在OpenClaw对话中输入：

从 https://gitee.com/bobsharon/JarvisMolt-Skills 学习lark技能
```

### 步骤3：输入授权码

```
当提示输入授权码时，直接输入：

ABCD-EFGH-JKLM-NPQR-XY
```

---

## 📝 开发笔记

### Agent设计说明

1. **状态管理**：使用`context.previousContext`保存中间状态
2. **授权缓存**：存储在`~/.openclaw/licenses/`
3. **临时目录**：使用`os.tmpdir()`自动清理
4. **错误处理**：所有操作都包含try-catch

### 改进建议

- [ ] 添加进度条显示
- [ ] 支持断点续传
- [ ] 添加技能依赖检查
- [ ] 实现技能版本管理
- [ ] 支持私有 Gitee 仓库

---

**测试完成后记得清理：**

```bash
# 清理测试数据
rm -rf ~/.openclaw/licenses/lark.json
rm -rf ~/.openclaw/skills/lark
```

---

**项目路径**: `Skills/skill-installer`
**最后更新**: 2026-02-17
